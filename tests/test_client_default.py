...
class TestAsync(ClientTest):
    ...
    async def test_table_delete(self):
        ...
        op = await self.aclient.table(self.table).delete().eq("name", "Canada").execute()
        self.assertIsNotNone(op)
        for d in op.data:
            self.assertEqual(d.get('name'), "Canada")
    ...
    async def test_table_select(self):
        ...
        op = await self.aclient.table(self.table).select("*").eq("name", "Canada").execute()
        self.assertIsNotNone(op)
        self.assertEqual(len(op.data), 1)
    ...
    async def test_table_update(self):
        with self.assertRaises(APIError):
            ...
            await self.aclient.table(self.table).update({"foo": "bar"}).eq("name", "Canada").execute()
            await self.aclient.table(self.table).update({"name": "Australia"}).eq("foo", "bar").execute()

        self.countries.append("Denmark")
        op = await self.aclient.table(self.table).update({"name": "Denmark"}).eq("name", "Dubai").execute()
        self.assertIsNotNone(op)
        for d in op.data:
            self.assertEqual(d.get('name'), "Denmark")
